...
import ExplorationSpots from "./ExplorationSpots";
...
export default function GameScene({ avatarModelPath }) {
  ...
  const [pendingSpotGeneration, setPendingSpotGeneration] = useState(true);
  ...
  const startGame = async () => {
    ...
    try {
      ...
      setFeedback(`Started in local mode (backend reset skipped): ${error.message}`);
    } finally {
  ...
  return (
    <div className="game-shell">
      <Canvas camera={{ position: [0, 3, 8], fov: 60 }}>
        ...
        <hemisphereLight skyColor="#bfd6f6" groundColor="#344038" intensity={0.7} />
      ...
      {startingGame && (
        <div className="game-loading-overlay" aria-live="polite">
          <div className="game-loading-overlay__card">
            <span className="game-loading-overlay__eyebrow">AI Escape Room</span>
            <h2>Preparing the room</h2>
            <p>Resetting puzzle state and generating the first encounter.</p>
          </div>
        </div>
      )}
    </div>
